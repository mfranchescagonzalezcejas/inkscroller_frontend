...
void main() {
  group('Issue #1: Android package identity', () {
    test('Gradle declares deterministic package IDs for every flavor', () {
      ...
      final defaultConfigBlock = _requiredBlockFor(gradle, 'defaultConfig');
      ...
      expect(
        _assignedString(defaultConfigBlock, 'applicationId'),
        _basePackageId,
      );

      final devSuffix = _assignedString(devFlavorBlock, 'applicationIdSuffix');
      final stagingSuffix = _assignedString(
        stagingFlavorBlock,
        'applicationIdSuffix',
      );
      ...
      expect(
        _effectivePackageId(_basePackageId, stagingSuffix),
        _stagingPackageId,
      );
    ...
    test('MainActivity exists only at the approved package path', () {
      ...
      final oldTemplateMainActivity = File(
        'android/app/src/main/kotlin/com/example/inkscroller/MainActivity.kt',
      );
      final oldFlutterTemplateMainActivity = File(
      ...
      expect(oldTemplateMainActivity.existsSync(), isFalse);
      expect(oldFlutterTemplateMainActivity.existsSync(), isFalse);
    });
  });
}

String _requiredBlockFor(String source, String blockName) {
  final block = _blockFor(source, blockName);

  if (block == null) {
    fail('Missing required Android Gradle block: $blockName');
  }
...
String _bracedBlockFrom(String source, int openingBraceIndex) {
  ...
  fail('Unclosed Android Gradle block starting at index $openingBraceIndex');
}
