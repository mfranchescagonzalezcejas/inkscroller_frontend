...
class _A extends Mock implements SignIn {} class _B extends Mock implements SignUp {} class _C extends Mock implements SignOut {} class _D extends Mock implements GetAuthState {} class _E extends Mock implements GetUserProfile {} class _I extends Mock implements UpdateUserProfile {} class _J extends Mock implements SendEmailVerification {} class _L extends Mock implements SendPasswordReset {} class _K extends Mock implements ReloadUser {} class _F extends Mock implements GetPreferences {} class _G extends Mock implements UpdatePreferences {} class _H extends Mock implements LibraryRemoteDataSource {}
...
void main() {
  ...
  test('adult with supported capability may select unspecified', () async {
    ...
    final auth = AuthNotifier(signIn: _A(), signUp: _B(), signOut: _C(), getAuthState: d, sendEmailVerification: _J(), sendPasswordReset: _L(), reloadUser: _K(), getUserProfile: _E(), updateUserProfile: _I())..state = const AuthState(user: AppUser(uid: 'user-1', email: 'adult@example.test'));
    final prefs = PreferencesNotifier(getPreferences: _F(), updatePreferences: _G());
    final profile = UserProfileNotifier(getUserProfile: _E(), updateUserProfile: _I())..state = UserProfileState(profile: UserProfile(firebaseUid: 'user-1', email: 'adult@example.test', birthDate: DateTime(1990), createdAt: DateTime(2020)));
  ...
  Future<ProviderContainer> containerFor({required AppUser? user, required DateTime? birthDate, required bool fails}) async {
      final d = _D(); when(() => d()).thenAnswer((_) => const Stream<AppUser?>.empty());
      final auth = AuthNotifier(signIn: _A(), signUp: _B(), signOut: _C(), getAuthState: d, sendEmailVerification: _J(), sendPasswordReset: _L(), reloadUser: _K(), getUserProfile: _E(), updateUserProfile: _I())..state = AuthState(user: user);
      final prefs = PreferencesNotifier(getPreferences: _F(), updatePreferences: _G());
      final profile = UserProfileNotifier(getUserProfile: _E(), updateUserProfile: _I())..state = UserProfileState(profile: user == null ? null : UserProfile(firebaseUid: user.uid, email: user.email, birthDate: birthDate, createdAt: DateTime(2020)));
