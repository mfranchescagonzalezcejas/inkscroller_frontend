...
- **Verificación**:
  - **Android**: `applicationId` efectivo del flavor `pro` actualizado a `dev.devdigi.inkscroller` para coincidir con la identidad existente en Google Play Console. Los archivos reales `google-services.json` no están versionados; antes de compilar con Firebase, provisionar `android/app/src/pro/google-services.json` con `package_name: dev.devdigi.inkscroller` y los equivalentes para `dev`/`staging` (`.dev`/`.stg`).
